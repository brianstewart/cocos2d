...
-(id) init
{
	if( (self=[super init])) {
        ...
        // Batch up all of the drawing of objects from the same image
        _batchNode = [CCSpriteBatchNode batchNodeWithFile:@"Sprites.pvr.ccz"]; 
        
        
        // Add the CCSpriteBatchNode to the layer to be drawn
        [self addChild:_batchNode];
        
        // Loads the plist file with info where the smaller images lie
        [[CCSpriteFrameCache sharedSpriteFrameCache] addSpriteFramesWithFile:@"Sprites.plist"];
        
        // Create a new sprite to hold the ship image
    ...
    for (CCSprite *asteroid in _asteroids) {        
        ...
        if (CGRectIntersectsRect(_ship.boundingBox, asteroid.boundingBox)) {
            ...
            [_ship runAction:[CCSequence actions:
                              [CCBlink actionWithDuration:1.0 blinks:9],
                              [CCCallFuncN actionWithTarget:self selector:@selector(setAlive:)],
                              nil]];
            _lives--;
        }
    }
